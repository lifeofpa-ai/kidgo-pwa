      ...
      <div className="max-w-2xl mx-auto px-4 py-6 sm:py-10">
        ...
        <div className="mb-8 card-enter">
          ...
          <p className="text-[var(--text-muted)] text-sm">
            Dein persönliches Familien-Dashboard
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          ...
          <div className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border)] p-4 card-enter" style={{ animationDelay: "80ms" }}>
            ...
            <Link
              href="/badges"
              className="text-xs text-[var(--accent)] hover:underline mt-2 block transition"
            >
              Alle ansehen
            </Link>
          </div>
              ...
              <div className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border)] hover:border-[var(--accent)] p-5 transition-all hover:shadow-sm">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-sm text-[var(--text-primary)] truncate group-hover:text-[var(--accent)] transition-colors">
                      {nextEvent.titel}
                    </p>
                    {nextEvent.ort && (
                      <p className="text-xs text-[var(--text-muted)] truncate mt-0.5">{nextEvent.ort}</p>
                    )}
                  </div>
                  {nextEvent.datum && (
          ...
          <div className="mb-6 card-enter" style={{ animationDelay: "320ms" }}>
            <div className="flex items-center justify-between mb-3 px-0.5">
              <p className="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wider">
                Gemerkte Events ({bookmarks.length})
              </p>
              <Link href="/" className="text-xs text-[var(--accent)] hover:underline transition">
                Alle ansehen
              </Link>
            </div>
            <div className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border)] divide-y divide-[var(--border)] overflow-hidden">
              {bookmarks.slice(0, 5).map((bm) => (
                <Link
                  key={bm.id}
                  href={`/events/${bm.id}`}
                  className="flex items-center gap-3 px-5 py-3.5 hover:bg-[var(--bg-subtle)] transition group"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {bm.datum && (
                      <p className="text-xs text-[var(--text-muted)] mt-0.5">{formatDateShort(bm.datum)}</p>
                    )}
                  </div>
              ...
              {bookmarks.length > 5 && (
                <div className="px-5 py-3 text-center">
                  <span className="text-xs text-[var(--text-muted)]">
                    +{bookmarks.length - 5} weitere Events
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border)] divide-y divide-[var(--border)] overflow-hidden">
              {reviews.map((review) => (
                <Link
                  ...
                  className="flex items-start gap-3 px-5 py-4 hover:bg-[var(--bg-subtle)] transition group"
                >
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-[var(--text-primary)] truncate group-hover:text-[var(--accent)] transition-colors">
                      {Array.isArray(review.events)
                        ? review.events[0]?.titel ?? "Event"
                        : review.events?.titel ?? "Event"}
                    </p>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6 card-enter" style={{ animationDelay: "400ms" }}>
          <Link
            href="/badges"
            className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border)] p-4 hover:border-[var(--accent)] transition group"
          >
